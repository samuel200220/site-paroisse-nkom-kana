...
export default function AdminDashboard() {
    ...
    return (
        <div>
            <div className="mb-12">
                <h1 className="text-3xl font-bold text-stone-900 dark:text-stone-100 mb-2 transition-colors">Tableau de Bord</h1>
                <p className="text-stone-500 dark:text-stone-400">Gérez le contenu de votre Église depuis cet espace.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                {stats.map((stat) => (
                    <div key={stat.name} className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm transition-colors">
                        <div className={`w-12 h-12 ${stat.color} text-white rounded-xl flex items-center justify-center mb-4 shadow-lg shadow-${stat.color.split('-')[1]}-200 dark:shadow-none`}>
                            {stat.icon}
                        </div>
                        <p className="text-stone-400 text-sm font-medium uppercase tracking-widest">{stat.name}</p>
                        <div className="flex items-end justify-between">
                            <span className="text-4xl font-bold text-stone-900 dark:text-stone-100">{stat.count}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-stone-900 rounded-3xl border border-stone-200 dark:border-stone-800 overflow-hidden shadow-sm transition-colors">
                    <div className="p-6 border-b border-stone-50 dark:border-stone-800 flex items-center justify-between">
                        ...
                        <Link href="/admin/annonces" className="text-amber-600 dark:text-amber-500 text-xs font-bold hover:underline">Voir tout</Link>
                    </div>
                    <div className="p-0">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="px-6 py-4 flex items-center gap-4 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors border-b border-stone-50 dark:border-stone-800 last:border-0">
                                ...
                                <div className="flex-grow">
                                    <h4 className="text-sm font-bold text-stone-900 dark:text-stone-100">Nouvelle annonce publiée</h4>
